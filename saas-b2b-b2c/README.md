...
Создайте `.env` в корне проекта (не добавляйте его в репозиторий). Переменные, содержащие секреты (например `JWT_SECRET`), рекомендуется задавать через **GitHub Secrets** и передавать в контейнер через `docker-compose` или CI.
...
PORT=8080
# JWT_SECRET берётся из GitHub Secrets; задайте её в .env только локально
