...
const SalesDynamicsChart: React.FC<SalesDynamicsChartProps> = ({ data, view = 'monthly' }) => {
  ...
  const processedData = React.useMemo(() => {
    ...
    if (currentView === 'quarterly') {
      ...
      for (let i = 0; i < data.length; i += 3) {
        ...
        const month = slice[0].month;
        const agg: SalesData = {
          month,
          plan: slice.reduce((s, cur) => s + cur.plan, 0),
          fact: slice.reduce((s, cur) => s + (cur.fact ?? 0), 0) || null,
          forecast: slice.reduce((s, cur) => s + (cur.forecast ?? 0), 0) || null,
        };
        result.push(agg);
      }
      return result;
  ...
  return (
    <div>
      <Radio.Group value={currentView} onChange={e => setCurrentView(e.target.value)} style={{ marginBottom: 16 }}>
