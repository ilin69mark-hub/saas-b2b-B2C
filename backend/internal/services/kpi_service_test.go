...
func (m *MockScheduleRepository) GetScheduleByFilters(ctx context.Context, userID *uuid.UUID, salonID *uuid.UUID, startDate, endDate time.Time) ([]models.Schedule, error) {
	args := m.Called(ctx, userID, salonID, startDate, endDate)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
type ScheduleRepositoryInterface interface {
	GetScheduleByFilters(ctx context.Context, userID *uuid.UUID, salonID *uuid.UUID, startDate, endDate time.Time) ([]models.Schedule, error)
}

type KPIServiceWithInterfaces struct {
	DB       interface{}
	kpiRepo  KPIRepositoryInterface
	schedRep ScheduleRepositoryInterface
}

func NewKPIServiceWithInterfaces(kpiRepo KPIRepositoryInterface, schedRep ScheduleRepositoryInterface) *KPIServiceWithInterfaces {
	return &KPIServiceWithInterfaces{
		kpiRepo:  kpiRepo,
		schedRep: schedRep,
	}
}

func (s *KPIServiceWithInterfaces) SetGoal(ctx context.Context, goal *models.DailyGoal) error {
	return s.kpiRepo.UpsertGoal(ctx, goal)
}

func (s *KPIServiceWithInterfaces) GetKPISummary(ctx context.Context, userID *uuid.UUID, salonID *uuid.UUID, startDate, endDate time.Time) (*models.KPISummary, error) {
...
func TestKPIService_SetGoal(t *testing.T) {
	t.Run("success creates new goal", func(t *testing.T) {
		...
		service := NewKPIServiceWithInterfaces(kpiRepo, schedRepo)
